// src/app/api/profile/evaluate/route.ts — POST: AI coaching feedback on a hand-written draft
...
import { EvaluateProfileTextUseCase } from "@/application/identity/commands/EvaluateProfileText";
import { evaluateProfileTextSchema } from "@/lib/validators/profile.schema";
...
export async function POST(req: NextRequest) {
  ...
  const parsed = evaluateProfileTextSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json({ error: "Validation failed.", details: parsed.error.flatten() }, { status: 400 });
  }

  const useCase = new EvaluateProfileTextUseCase(
