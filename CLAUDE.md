...
2. **AI Core** — a separate local-first AI platform (its own repo, not part of this codebase) that AI features call over HTTP. It is *not* started by `npm run dev`; it must be run independently (`uvicorn app.main:app --reload --port 8001` from AI Core's `backend/`, after `docker compose up -d` for its Postgres/Chroma and `alembic upgrade head`). Configure `AI_CORE_BASE_URL` / `AI_CORE_API_KEY` in `.env`.

AI provider chain (`GeminiClient.ts`): AI Core first; if it's unreachable/unset, fall back to the Google Gemini API directly — the per-user key saved in Profile → AI settings (`User.geminiApiKey`, read via `withUserAiContext` AsyncLocalStorage in `src/infrastructure/ai/requestAiContext.ts`; every AI route wraps its use-case call in it) takes priority over the deployment's `GEMINI_API_KEY` env. With neither AI Core nor a Gemini key, AI routes throw.
...
- **AI services** (`src/infrastructure/ai/gemini/*Service.ts`) all funnel through `geminiComplete()` in `GeminiClient.ts` (which owns the AI Core → Gemini provider chain). Model responses come back as markdown-fenced JSON and must be stripped/parsed defensively (see `JobAnalyzerService.parseResponse`) — follow that pattern for new AI calls rather than assuming clean JSON. New AI routes must wrap their use-case call in `withUserAiContext(userId, ...)` so per-user BYOK keys apply.
...
Vercel. `vercel.json` sets `maxDuration: 60` on all AI-backed routes since AI calls run long. AI Core has no permanent public hosting; in production it's reached through a Cloudflare quick tunnel to the dev machine (URL changes on every tunnel restart — update `AI_CORE_BASE_URL` in Vercel when it does), and when the tunnel is down the Gemini fallback (env `GEMINI_API_KEY` or per-user BYOK) keeps AI features working.
